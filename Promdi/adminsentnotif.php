...
<?php
$id=$_GET["ID"];
	$query=$conn->prepare("select *, tblmessages.ID AS mid from tblmessages LEFT JOIN tblaccounts ON RecieverID = tblaccounts.ID where tblmessages.ID=:id");
...
?>
	...
	<p class='postinfo'>Sent on ".$row["Date"]."</p>
	<p class='postinfo'>To ".$row["Username"]."</p>
