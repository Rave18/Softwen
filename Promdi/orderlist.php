...
<div id="wrapper">
	...
	<h3 style="font-family:Cambria;text-align:center">Orders</h3></br>
	 ...
	 <?php
		...
		global $query;
		$id=$_GET["id"];
		$query=$conn->prepare("select *, tblorderdetails.Quantity AS Qty from tblorderdetails left join tblproducts on productid=tblproducts.id where orderid=:id;");
	 ...
	 while($row=$query->fetch(PDO::FETCH_ASSOC)){
		 echo "<tr><td>".$row["ID"]."</td><td>".$row["ProductName"]."</td><td>".$row["Qty"]."</td><td>".$row["Price"]."</td></tr>";
