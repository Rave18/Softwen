...
<div id="wrapper">
	...
	<h3 style="font-family:Cambria;text-align:center"><a href="notif.php">Inbox</a>      |     <a href="usersent.php">Sent</a>      </h3>
		...
		<?php
$id=$_GET["ID"];
	$query=$conn->prepare("select *, tblnotif.ID AS nid from tblnotif LEFT JOIN tblaccounts ON AccountID = tblaccounts.ID where tblnotif.id=:id");
...
?>
	...
	<p class='postinfo'>Recieved on ".$row["Date"]."</p>
	<p class='postinfo'>From ".$row["Username"]."</p>
	<p id='parag'>".$row["Body"]."</p>
</article>
";
		}
	?>
