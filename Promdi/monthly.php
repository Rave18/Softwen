...
?>
 ...
 <div class="jumbotron" id="total_annually" style="border-radius: 8px;padding: 10px; margin-top: 5px;box-shadow: 0 3px 8px 0 rgba(0, 0, 0, 0.6);">
    ...
    include "connection.php";
       $qry=$conn->prepare("SELECT  SUM(Quantity*Price) as TOTAL FROM tblorders inner join tblorderdetails on tblorders.ID = orderid  WHERE YEAR(ShippingDate) = YEAR(NOW()) AND MONTH(ShippingDate) = MONTH(NOW()) ");
        $qry->execute();
        $result=$qry->rowCount();
