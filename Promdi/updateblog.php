...
<?php
		...
		$query=$conn->prepare("select * from tblblogs ORDER BY tblblogs.ID DESC");
		$query->execute();
		$rows = $query->fetchAll(PDO::FETCH_ASSOC);
		foreach($rows as $row){
			
		$cont=strip_tags($row["BlogBody"]);
...
<section class='Post-Heading'>
		...
		<h3>".$row["BlogTitle"]."</h3>
<small>Posted on ".$row["BlogDate"]."</small>
</section>
<div id='Post-Message'>
<p> $conte . . .<a href='updateblogdesc.php?ID=".$row["ID"]."'></br>Update Blog	</p></a>
</div>
</article>";
		}
?>
