...
<div id="wrapper">
	...
	<article>
		...
		<div id="annualy_reports" style='clear: left; overflow-y: auto; margin-top:10px;overflow-x: auto; min-height: 400px; max-height: 400px;  box-shadow: 0 3px 8px 0 rgba(0, 0, 0, 0.6);border-radius: 8px;  '>
<?php
 include "connection.php";
  global $qry;
  global $result;
  $qry=$conn->prepare("SELECT AccountID,FirstName,LastName,City,Province,ContactNumber,ShippingDate,  SUM(Quantity * Price)as TOTAL FROM tblorders inner join tblorderdetails on tblorders.ID=tblorderdetails.OrderID   WHERE YEAR(ShippingDate) = YEAR(NOW()) AND Status = 'Paid' GROUP BY tblorderdetails.OrderID ORDER BY ShippingDate ASC");
  $qry->execute();
  $result=$qry->rowCount();
...
?>
 ...
 <div class="jumbotron" id="total_annually" style="border-radius: 8px;padding: 10px; margin-top: 5px;box-shadow: 0 3px 8px 0 rgba(0, 0, 0, 0.6);">
    ...
    include "connection.php";
       $qry=$conn->prepare("SELECT SUM(Quantity * Price)as TOTAL FROM tblorders INNER JOIN tblorderdetails ON tblorders.ID = tblorderdetails.OrderID WHERE YEAR(ShippingDate) = YEAR(NOW())");
        $qry->execute();
        $result=$qry->rowCount();
