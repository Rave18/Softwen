...
session_destroy();
	...
	if(isset($_POST["edit"])){
		...
		$query=$conn->prepare("update tblBlogs set BlogTitle=:blogtitle, BlogDate=:blogdate, BlogBody=:blogbody where ID=:id");
...
<div id="wrapper">
	...
	<?php
	$id=$_GET["ID"];
	$query=$conn->prepare("Select * from tblblogs where ID=:id");
	$query->bindParam("id",$id);
	$query->execute();
...
<fieldset>
<h3 style="font-family:Cambria;text-align:center">UPDATE BLOG</h3></br>
<input type="text" name="BlogTitle" placeholder="Title" size="35" id="blogtitle" value="<?php print $row["BlogTitle"];?>" style="margin:5px:" required/><br>
<input type="date" name="BlogDate"  id="date" size="35" value=<?php print $row["BlogDate"];?> required/><br>
<textarea name="BlogBody" placeholder="Write your blog here" size="300" id="blogbody" style="margin:5px:"/ ><?php print $row["BlogBody"]; ?></textarea>
	 <?php } ?>
