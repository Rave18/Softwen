...
session_destroy();
	...
	if(isset($_POST["button1"])){
		...
		$query=$conn->prepare("select password from tblaccounts where Password=:oldpass");
		$query->bindParam("oldpass",$oldpass);
		$query->execute();
		$result=$query->rowCount();
	if($result){
		if($newpass==$confpass)
		{
			$query=$conn->prepare("update tblaccounts set Password=:Password where ID=:id");
		$query->bindParam("Password",$confpass);
		$query->bindParam("id",$_SESSION["ID"]);
		
		if($query->execute()){
		$error="Your password had been changed successfully";

		}
		
		}
		else{
		$eror="Your new password does not match";
	}
...
<div id="wrapper">
	...
	<li><a href="addblog.php" class="under"><strong>WRITE A BLOG</strong></a></li>
		...
		<h3 style="font-family:Cambria;text-align:center"><a href="">Change My Password</a>|<a href="makeadmin.php">Create Admin</a></h3>
...
<fieldset>
<h3 style="font-family:Cambria;text-align:center">CHANGE PASSWORD</h3></br>

<form action="/action_page.php">
<input type="password" name="OldPass" placeholder="Old Password" size="35" id="prod1" style="margin:5px:" required/><br>
<input type="password" name="NewPass"  minlength=8 placeholder="New Password" size="35" id="prod1" style="margin:5px:" required/><br>
<input type="password"  name="ConfirmPass"  minlength=8 placeholder="Confirm New Password"  id="prod3" size="35" style="margin:5px:" required/><br>
<strong><input type="submit" id="addprod" value="Change Password" name="button1"></strong>
</form>
</fieldset>
</form>
</article>
