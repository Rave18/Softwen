...
?>
 ...
 <div class="jumbotron" id="total_annually" style="border-radius: 8px;padding: 10px; margin-top: 5px;box-shadow: 0 3px 8px 0 rgba(0, 0, 0, 0.6);">
    ...
    include "connection.php";
       $qry=$conn->prepare("SELECT SUM(Quantity*Price) AS TOTAL FROM tblorders  inner join tblorderdetails on tblorders.id=orderid WHERE DATE(ShippingDate) BETWEEN DATE(NOW()) AND DATE(DATE_ADD(NOW(), INTERVAL 7 DAY)) ");
        $qry->execute();
        $result=$qry->rowCount();
